...
export function getNotices(): Notice[] { return loadNotices(); }
export function saveNoticesPublic(list: Notice[]) { saveNotices(list); }
...
export default function NoticePage() {
  ...
  useEffect(() => {
    const loaded = loadNotices();
    setNotices(loaded);
    // 공지사항 페이지 방문 시 마지막 확인 ID 업데이트 → 배지 초기화
    if (loaded.length) {
      const maxId = Math.max(...loaded.map(n => n.id));
      localStorage.setItem('taes-notice-last-seen', String(maxId));
    }
  }, []);

  function persist(next: Notice[]) { setNotices(next); saveNotices(next); }

  function handleSubmit() {
    if (!form.title.trim() || !form.author.trim()) return;
    const today = new Date().toISOString().slice(0, 10).replace(/-/g, '.');
    const notice: Notice = { id: Date.now(), ...form, title: form.title.trim(), content: form.content.trim(), author: form.author.trim(), date: today, views: 0 };
    persist([notice, ...notices]);
    setShowWrite(false);
    setForm(emptyForm);
    setPage(1);
  }
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-8">
          <div className="flex flex-wrap gap-2">
            {CATEGORIES.map(cat => (
              <button
                ...
                style={{
                  backgroundColor: activeCategory === cat ? '#CC0000' : '#121212',
          ...
          <input
            ...
            className="sm:ml-auto px-4 py-2 text-sm text-white bg-[#121212] border border-white/10 focus:border-red-700 outline-none w-full sm:w-64"
          />
        </div>
        ...
        {normal.length > 0 && (
          <div className="border border-white/10">
            <div className="grid grid-cols-12 py-3 px-5 text-xs font-bold text-white/40 uppercase tracking-wider border-b border-white/10" style={{ backgroundColor: '#0e0e0e' }}>
          ...
          <div className="flex justify-center gap-2 mt-8">
            {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
              <button
                ...
                style={{
                  backgroundColor: p === page ? '#CC0000' : '#121212',
      ...
      {showWrite && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ backgroundColor: 'rgba(0,0,0,0.88)' }}>
          <div className="w-full max-w-lg p-6 border border-white/10 overflow-y-auto max-h-[90vh]" style={{ backgroundColor: '#0e0e0e' }}>
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => { setShowWrite(false); setForm(emptyForm); }} className="flex-1 py-2 text-sm font-bold text-white/50 border border-white/10 hover:border-white/30 transition-colors">취소</button>
              <button
